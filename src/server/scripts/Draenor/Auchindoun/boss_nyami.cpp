                ...
                if (Creature* l_Warden = l_Instance->instance->GetCreature(l_Instance->GetData64(eAuchindounDatas::DataWarden)))
                {
                    if (l_Warden->IsAIEnabled)
                    {
                        switch (m_Modifier)
                        {
                            ...
                            case 1:
                            {
                                if (GameObject* l_Door = l_Instance->instance->GetGameObject(l_Instance->GetData64(eAuchindounDatas::DataAuchindounWindow)))
                                {
                                    l_Door->SetLootState(LootState::GO_READY);
                                    l_Door->UseDoorOrButton(10 * TimeConstants::IN_MILLISECONDS);
                                }
                                ...
                                l_Warden->SummonCreature(eAuchindounCreatures::CreatureFelborneAbyssal, l_Warden->GetPositionX(), l_Warden->GetPositionY(), l_Warden->GetPositionZ(), TEMPSUMMON_MANUAL_DESPAWN);
...
public:
    ...
    boss_nyami() : CreatureScript("auchindoun_boss_nyami") { }
    ...
    struct boss_nyamiAI : public BossAI
    {
        ...
        void Reset() override
        {
            ...
            {
                m_First = true;
                me->SummonCreature(eAuchindounCreatures::CreatureWardenAzzakael, g_PositionWardenSpawnPoint);
            }      
        }
        ...
        void JustDied(Unit* /*p_Killer*/) override
        {
            ...
            if (m_Instance != nullptr)
            {
                ...
                me->m_Events.AddEvent(new EventPostNyamiFight(me, 10), me->m_Events.CalculateTime(5 * TimeConstants::IN_MILLISECONDS));
            }          
        }
...
public:
    ...
    struct auchindoun_nyami_mob_warden_cosmeticAI : public ScriptedAI
    {
        ...
        void MovementInform(uint32 /*p_Type*/, uint32 p_Id) override
        {
            switch (p_Id)
            {
                case eNyamiMovementInformed::MovementWardenMoveOuttaGate:
            ...
            switch (events.ExecuteEvent())
            {
                ...
                case eNyamiEvents::EventRadiantFury:
                    if (Unit* l_Target = SelectTarget(SelectAggroTarget::SELECT_TARGET_FARTHEST, 0, 50.0f, true))
                    {
                        m_Radiant = true;
                        m_RadiantDiff = 1 * TimeConstants::IN_MILLISECONDS;
                        me->GetMotionMaster()->MoveJump(l_Target->GetPositionX(), l_Target->GetPositionY(), l_Target->GetPositionZ(), 4.0f, 4.0f, 10.0f, eNyamiMovementInformed::MovementInformedRadiantFury);
                    }
...
/// Radiant Fury - 432626
class auchindoun_nyami_mob_radiant_fury : public CreatureScript
{
public:

    auchindoun_nyami_mob_radiant_fury() : CreatureScript("auchindoun_nyami_mob_radiant_fury") { }

    struct auchindoun_nyami_mob_radiant_furyAI : public Scripted_NoMovementAI
    {
        auchindoun_nyami_mob_radiant_furyAI(Creature* p_Creature) : Scripted_NoMovementAI(p_Creature)
        {
            m_Instance = me->GetInstanceScript();  
        }
        ...
        void Reset() override
        {
            me->CastSpell(me, eNyamiSpells::SpellRadiantFullyVisual);
    ...
    CreatureAI* GetAI(Creature* p_Creature) const override
    {
        return new auchindoun_nyami_mob_radiant_furyAI(p_Creature);
    }
};
...
public:
    ...
    struct auchindoun_nyami_mob_twisted_magusAI : public ScriptedAI
    {
        ...
        void EnterCombat(Unit* p_Attacker) override
        {
            ...
            events.ScheduleEvent(eTwistedMagusEvents::EventArcaneBomb, 10 * TimeConstants::IN_MILLISECONDS);
        }
        ...
        void UpdateAI(const uint32 p_Diff) override
        {
            ...
            switch (events.ExecuteEvent())
            {
                case eTwistedMagusEvents::EventArcaneBolt:
                        ...
                        events.ScheduleEvent(eTwistedMagusEvents::EventArcaneBolt, 6 * TimeConstants::IN_MILLISECONDS);
                        break;
    ...
    void OnUpdate(AreaTrigger* p_AreaTrigger, uint32 p_Time) override
    {
        if (m_Diff <= p_Time)
        {
            ...
            if (!l_ListPlayers.empty())
            {
                ...
                {
                    if ((*l_Itr) && (*l_Itr)->IsInWorld())
                        (*l_Itr)->CastSpell((*l_Itr), eNyamiSpells::SpellRadiantDamage);
                }
            }
...
void AddSC_boss_nyami()
{
    ...
    new auchindoun_nyami_mob_radiant_fury();            ///< 432626
