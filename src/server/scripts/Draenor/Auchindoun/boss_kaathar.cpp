                        ...
                        if (Creature* l_Kaathar = l_Instance->instance->GetCreature(l_Instance->GetData64(eAuchindounDatas::DataBossKathaar)))
                        {
                            if (l_Tuulani->GetAI() && l_Nyami->GetAI() &&  l_Kaathar->GetAI())
                            {
                                switch (m_Modifier)
                                {
                                    case 100:
                                    {
                                        l_Nyami->AI()->Talk(eAuchindounTalk::NYAMITALK6);
                                    ...
                                    case 101:
                                    {
                                        l_Nyami->AI()->Talk(eAuchindounTalk::NYAMITALK7);
                                    ...
                                    case 0:
                                    {
                                        l_Nyami->AI()->Talk(eAuchindounTalk::NYAMITALK8);
                                        l_Nyami->GetMotionMaster()->MovePoint(eAuchindounMovementInform::MovementInformNyamiEscape05, g_PositionNyamiEscape01);
                                        break;
                                    }
                                    case 1:
                                    {
                                        l_Nyami->GetMotionMaster()->MovePoint(eAuchindounMovementInform::MovementInformNyamiEscape06, g_PositionNyamiEscape02);
                                        break;
                                    }
                                    case 2:
                                    {
                                        ...
                                        l_Nyami->GetMotionMaster()->MovePoint(eAuchindounMovementInform::MovementInformNyamiEscape07, g_PositionNyamiEscape03);
                                    ...
                                    case 3:
                                    {
                                        l_Nyami->GetMotionMaster()->MovePoint(eAuchindounMovementInform::MovementInformNyamiEscape08, g_PositionNyamiEscape04);
                                        break;
                                    }
                                    case 4:
                                    {
                                    ...
                                    case 6:
                                    {
                                        l_Tuulani->AI()->Talk(eAuchindounTalk::TUULANITALK10);
                                    ...
                                    case 7:
                                    {
                                        l_Tuulani->AI()->Talk(eAuchindounTalk::TUULANITALK11);
                                    ...
                                    case 8:
                                    {
                                        l_Tuulani->AI()->Talk(eAuchindounTalk::TUULANITALK9);
                                        l_Tuulani->AddAura(eAuchindounSpells::SpellTuulaniCapturedVoidPrison, l_Tuulani);
                    ...
                    if (l_Tuulani->GetAI())
                    {
                        switch (m_Modifier)
                        {
                            case 0:
                            {
                                ...
                                l_Tuulani->RemoveAura(eAuchindounSpells::SpellTuulaniCapturedVoidPrison); // void prison

                                for (int32 i = 0; i < 4; i++)
                                {
                                    if (Creature* l_Defender = m_Obj->SummonCreature(eAuchindounCreatures::CreatureAucheniDefender, g_PositionFourMagesThatSpawnAfterKaatharIsKaaput[i], TempSummonType::TEMPSUMMON_DEAD_DESPAWN))
                                    {
                            ...
                            {                        
                                ...
                                for (int32 i = 0; i < 2; i++)
                                {
                                    if (Creature* l_Corpses = l_Tuulani->SummonCreature(eAuchindounCreatures::CreatureAucheniDefender, g_PositionThreeCorpses[i], TempSummonType::TEMPSUMMON_DEAD_DESPAWN))
                                    {
                                ...
                                Creature* l_Magus = l_Tuulani->SummonCreature(eAuchindounCreatures::CreatureSargereiMagus, g_PositionMagusAndDefenderHostile[0], TempSummonType::TEMPSUMMON_DEAD_DESPAWN);   
                                {
                                ...
                                for (int32 i = 0; i < 2; i++)
                                {
                                    l_Tuulani->SummonCreature(eAuchindounCreatures::CreatureSargereiDefender, g_PositionSargereiDefenders[i], TempSummonType::TEMPSUMMON_DEAD_DESPAWN);
                                }

                                // Magus p_Who control footmans                            
                                if (Creature* l_Magus = l_Tuulani->SummonCreature(eAuchindounCreatures::CreatureSargereiMagus, g_PositionMagusp_WhoControlFootmans, TempSummonType::TEMPSUMMON_DEAD_DESPAWN))
                                {
                                    l_Magus->CastSpell(l_Magus, SpellArcaneChanneling);
                                }
                                ...
                                for (int32 i = 0; i < 3; i++)
                                {                           
                                    if (Creature* l_Prisoners = l_Tuulani->SummonCreature(eAuchindounCreatures::CreatureAucheniSoulPriest, g_PositionThreePrisoners[i], TempSummonType::TEMPSUMMON_DEAD_DESPAWN))
                                    {
                                        l_Prisoners->SetCanFly(true);
                                        l_Prisoners->SetDisableGravity(true);
                                        l_Prisoners->CastSpell(l_Prisoners, eAuchindounSpells::SpellPrisonAura);
                                        l_Prisoners->CastSpell(l_Prisoners, eAuchindounSpells::SpellStrangulate);
                                ...
                                {                              
                                    if (Creature* l_Prisoners = l_Tuulani->SummonCreature(eAuchindounCreatures::CreatureAucheniDefender, g_PositionCorpsesNearNyomi[i], TempSummonType::TEMPSUMMON_DEAD_DESPAWN))
                                    {
                                        l_Prisoners->SetReactState(ReactStates::REACT_PASSIVE);
                                        l_Prisoners->AddUnitMovementFlag(MovementFlags::MOVEMENTFLAG_ROOT);
                                        l_Prisoners->SetFlag(EObjectFields::OBJECT_FIELD_DYNAMIC_FLAGS, UnitDynFlags::UNIT_DYNFLAG_DEAD);
                                ...
                                for (int32 i = 0; i < 2; i++)
                                {
                                    l_Tuulani->SummonCreature(eAuchindounCreatures::CreatureSargereiDefender, g_PositionGuardsAndWardens[i], TempSummonType::TEMPSUMMON_DEAD_DESPAWN);
                                }
                                ...
                                for (int32 i = 0; i < 4; i++)
                                {
                                    if (Creature* l_Defenders = l_Tuulani->SummonCreature(eAuchindounCreatures::CreatureAucheniWarden, g_PositionDefenderBehindMiddleWallOfNyami[i], TempSummonType::TEMPSUMMON_DEAD_DESPAWN))
                                    {
                                ...
                                for (int32 i = 0; i < 2; i++)
                                {
                                    if (Creature* l_Defenders = l_Tuulani->SummonCreature(eAuchindounCreatures::CreatureAucheniWarden, g_PositionDefenderBehindBackWallOfNyami[i], TempSummonType::TEMPSUMMON_DEAD_DESPAWN))
                                    {
                                        l_Defenders->SetFlag(EUnitFields::UNIT_FIELD_FLAGS, eUnitFlags::UNIT_FLAG_IMMUNE_TO_PC | eUnitFlags::UNIT_FLAG_NON_ATTACKABLE | eUnitFlags::UNIT_FLAG_DISABLE_MOVE);
                                        l_Defenders->SetReactState(ReactStates::REACT_PASSIVE);
...
public:
    ...
    struct auchindoun_kaathar_mob_nyamiAI : public ScriptedAI
    {
        ...
        void MovementInform(uint32 /*p_Type*/, uint32 p_Id) override
        {
            switch (p_Id)
            {
                case eAuchindounMovementInform::MovementInformNyamiEscape05:
                    me->m_Events.AddEvent(new EventNyamiEscape(me, 1), me->m_Events.CalculateTime(200));
                    break;
                case eAuchindounMovementInform::MovementInformNyamiEscape06:
                    me->m_Events.AddEvent(new EventNyamiEscape(me, 2), me->m_Events.CalculateTime(200));
                    break;
                case eAuchindounMovementInform::MovementInformNyamiEscape07:
                    me->m_Events.AddEvent(new EventNyamiEscape(me, 3), me->m_Events.CalculateTime(200));
                    break;
                case eAuchindounMovementInform::MovementInformNyamiEscape08:
...
class auchindoun_boss_kaathar : public CreatureScript
{
public:
    auchindoun_boss_kaathar() : CreatureScript("auchindoun_boss_kaathar") { }
    ...
    struct boss_kaatharAI : public BossAI
    {
        ...
        void JustReachedHome() override
        {
            ...
            ActivateDoors();        
            if (m_Instance != nullptr)
                instance->SetBossState(eAuchindounDatas::DataBossKathaar, EncounterState::FAIL); 
        }
        ...
        void DoAction(int32 const p_Action) override
        {
            switch (p_Action)
            {
                case eAuchindounGeneralActions::ActionCountPre1StBossKill:
                    m_Counting = m_Counting + 1;
                    if (m_Counting >= 12)
                    {
                        me->RemoveAura(eAuchindounSpells::SpellGuard);

                        if (Creature* l_Tuulani = instance->instance->GetCreature(instance->GetData64(eAuchindounDatas::DataTuulani)))
                        {
                            if (Creature* l_Nyami = instance->instance->GetCreature(instance->GetData64(eAuchindounDatas::DataNyami)))
                            {
        ...
        {
            int32 l_Entries[14] = { 76595, eAuchindounCreatures::CreatureAuchenaiDefenderUnique, eAuchindounCreatures::CreatureAucheniMagus2, eAuchindounCreatures::CreatureAucheniVigiliant, eAuchindounCreatures::CreatureAucheniHoplite,
        ...
        void JustDied(Unit* /*p_Killer*/) override
        {
            ...
            if (m_Instance != nullptr)
                m_Instance->SetBossState(eAuchindounDatas::DataBossKathaar, EncounterState::DONE);
        ...
        void UpdateAI(uint32 const p_Diff) override
        {
            ...
            switch (events.ExecuteEvent())
            {
                ...
                case eKaatharEvents::EventHallowedGround:
                {
                    ...
                    if (Creature* l_Trigger = me->SummonCreature(eKaatharCreatures::TriggerHallowedGround, l_Position, TempSummonType::TEMPSUMMON_MANUAL_DESPAWN))
...
{
    new auchindoun_boss_kaathar();                                      ///< 75839
    ...
    new auchindoun_kaathar_at_fissure();
}