...
public:
    ...
    static bool HandleReloadCreatureTemplateCommand(ChatHandler* handler, const char* args)
    {
        ...
        for (Tokenizer::const_iterator itr = entries.begin(); itr != entries.end(); ++itr)
        {
            ...
            cInfo->faction             = fields[29].GetUInt16();
            cInfo->npcflag             = fields[30].GetUInt32();
            cInfo->npcflag2            = fields[31].GetUInt32();
            cInfo->speed_walk          = fields[32].GetFloat();
            cInfo->speed_run           = fields[33].GetFloat();
            cInfo->speed_fly           = fields[34].GetFloat();
            cInfo->scale               = fields[35].GetFloat();
            cInfo->rank                = fields[36].GetUInt8();
            cInfo->mindmg              = fields[37].GetFloat();
            cInfo->maxdmg              = fields[38].GetFloat();
            cInfo->dmgschool           = fields[39].GetUInt8();
            cInfo->attackpower         = fields[40].GetUInt32();
            cInfo->dmg_multiplier      = fields[41].GetFloat();
            cInfo->baseattacktime      = fields[42].GetUInt32();
            cInfo->rangeattacktime     = fields[43].GetUInt32();
            cInfo->unit_class          = fields[44].GetUInt8();
            cInfo->unit_flags          = fields[45].GetUInt32();
            cInfo->unit_flags2         = fields[46].GetUInt32();
            cInfo->dynamicflags        = fields[47].GetUInt32();
            cInfo->family              = fields[48].GetUInt8();
            cInfo->trainer_type        = fields[49].GetUInt8();
            cInfo->trainer_spell       = fields[50].GetUInt32();
            cInfo->trainer_class       = fields[51].GetUInt8();
            cInfo->trainer_race        = fields[52].GetUInt8();
            cInfo->minrangedmg         = fields[53].GetFloat();
            cInfo->maxrangedmg         = fields[54].GetFloat();
            cInfo->rangedattackpower   = fields[55].GetUInt16();
            cInfo->type                = fields[56].GetUInt8();
            cInfo->type_flags          = fields[57].GetUInt32();
            cInfo->type_flags2         = fields[58].GetUInt32();
            cInfo->lootid              = fields[59].GetUInt32();
            cInfo->pickpocketLootId    = fields[60].GetUInt32();
            cInfo->SkinLootId          = fields[61].GetUInt32();

            for (uint8 i = SPELL_SCHOOL_HOLY; i < MAX_SPELL_SCHOOL; ++i)
                cInfo->resistance[i] = fields[62 + i - 1].GetUInt16();

            cInfo->spells[0]           = fields[68].GetUInt32();
            cInfo->spells[1]           = fields[69].GetUInt32();
            cInfo->spells[2]           = fields[70].GetUInt32();
            cInfo->spells[3]           = fields[71].GetUInt32();
            cInfo->spells[4]           = fields[72].GetUInt32();
            cInfo->spells[5]           = fields[73].GetUInt32();
            cInfo->spells[6]           = fields[74].GetUInt32();
            cInfo->spells[7]           = fields[75].GetUInt32();
            cInfo->PetSpellDataId      = fields[76].GetUInt32();
            cInfo->VehicleId           = fields[77].GetUInt32();
            cInfo->mingold             = fields[78].GetUInt32();
            cInfo->maxgold             = fields[79].GetUInt32();
            cInfo->AIName              = fields[80].GetString();
            cInfo->MovementType        = fields[81].GetUInt8();
            cInfo->InhabitType         = fields[82].GetUInt8();
            cInfo->HoverHeight         = fields[83].GetFloat();
            cInfo->ModHealth           = fields[84].GetFloat();
            cInfo->ModMana             = fields[85].GetFloat();
            cInfo->ModManaExtra        = fields[86].GetFloat();
            cInfo->ModArmor            = fields[87].GetFloat();
            cInfo->RacialLeader        = fields[88].GetBool();
            cInfo->questItems[0]       = fields[89].GetUInt32();
            cInfo->questItems[1]       = fields[90].GetUInt32();
            cInfo->questItems[2]       = fields[91].GetUInt32();
            cInfo->questItems[3]       = fields[92].GetUInt32();
            cInfo->questItems[4]       = fields[93].GetUInt32();
            cInfo->questItems[5]       = fields[94].GetUInt32();
            cInfo->movementId          = fields[95].GetUInt32();
            cInfo->RegenHealth         = fields[96].GetBool();
            cInfo->equipmentId         = fields[97].GetUInt32();
            cInfo->MechanicImmuneMask  = fields[98].GetUInt32();
            cInfo->flags_extra         = fields[99].GetUInt32();
            cInfo->ScriptID            = sObjectMgr->GetScriptId(fields[100].GetCString());
