...
void WorldSession::DoLootRelease(uint64 lguid)
{
    ...
    else if (IS_CORPSE_GUID(lguid))        // ONLY remove insignia at BG
    {
        ...
        /// Check for Glyph of Fetch too
        if (!corpse || !corpse->IsWithinDistInMap(m_Player, INTERACTION_DISTANCE) && !m_Player->HasSpell(125050)) ///<  '&&' within '||'
            return;
    ...
    else
    {
        ...
        /// Check for Glyph of Fetch too
        if (!lootAllowed || !creature->IsWithinDistInMap(m_Player, INTERACTION_DISTANCE) && !m_Player->HasSpell(125050)) ///<  '&&' within '||'
            return;
