    ...
    */
    ServerPktHeader(uint32 size, uint32 cmd, AuthCrypt* _authCrypt) : size(size)
    {
        ...
        else
        {
            ...
            uint32 l_NewSize = size + 2;
            memcpy(&header[0], &l_NewSize, 2);
...
int WorldSocket::SendPacket(WorldPacket const& pct)
{
    ...
    bool l_ShouldCompress = false;// (m_Session && pkt->size() > 0x400);
    ...
    if (!l_ShouldCompress)
    {
        ServerPktHeader header(pkt->size(), pkt->GetOpcode(), &m_Crypt);
    ...
    if (m_Crypt.IsInitialized())
    {
        ...
        if ((header.size < 4) || (header.size > 20240) || (header.cmd >= 0xFFFF))
        {
            sLog->outError(LOG_FILTER_GENERAL, "WorldSocket::handle_input_header: client sent malformed packet size = %d , cmd = %d",
                (int)header.size, (int)header.cmd);
        ...
        ACE_NEW_RETURN(m_RecvWPct, WorldPacket(opcode, header.size), -1);
