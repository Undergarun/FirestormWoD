...
bool Unit::HandleAuraProcOnPowerAmount(Unit* victim, uint32 /*damage*/, AuraEffectPtr triggeredByAura, SpellInfo const *procSpell, uint32 procFlag, uint32 /*procEx*/)
{
...
//victim may be NULL
bool Unit::HandleDummyAuraProc(Unit* victim, uint32 damage, AuraEffectPtr triggeredByAura, SpellInfo const* procSpell, uint32 procFlag, uint32 procEx)
{
        ...
        case SPELLFAMILY_WARLOCK:
        {
            switch (dummySpell->Id)
            {
                ...
                case 108563:// Backlash
                {
                    ...
                    ToPlayer()->AddSpellCooldown(108563, 0, 8000);
                    break;
                }
        ...
        case SPELLFAMILY_DRUID:
        {
            switch (dummySpell->Id)
            {
                ...
                case 46832: // Sudden Eclipse (S12 - 2P Balance)
                {
                    ...
                    ToPlayer()->AddSpellCooldown(46832, 0, 6000);
        ...
        case SPELLFAMILY_ROGUE:
        {
            switch (dummySpell->Id)
            {
                case 51701: // Honor Among Thieves
                {
                    ...
                    {
                        player->AddSpellCooldown(51701, 0, 2000);
        ...
        case SPELLFAMILY_MONK:
        {
            switch (dummySpell->Id)
            {
                ...
                case 124502:// Gift of the Ox
                {
                    ...
                    ToPlayer()->AddSpellCooldown(124502, 0, 500); // Prevent multiple procs
                ...
                case 116023:// Sparring
                {
                    ...
                    ToPlayer()->AddSpellCooldown(116023, 0, 30 * IN_MILLISECONDS);
                ...
                case 116092:// Afterlife
                {
                    ...
                    ToPlayer()->AddSpellCooldown(116092, 0, 500);
...
bool Unit::HandleAuraProc(Unit* victim, uint32 /*damage*/, AuraPtr triggeredByAura, SpellInfo const* procSpell, uint32 /*procFlag*/, uint32 /*procEx*/, bool * handled)
{
    SpellInfo const* dummySpell = triggeredByAura->GetSpellInfo();
...
bool Unit::HandleProcTriggerSpell(Unit* victim, uint32 damage, AuraEffectPtr triggeredByAura, SpellInfo const* procSpell, uint32 procFlags, uint32 procEx)
{
    // Get triggered aura spell info
    SpellInfo const* auraSpellInfo = triggeredByAura->GetSpellInfo();
    ...
    switch (auraSpellInfo->Id)
    {
        ...
        case 31829: // Item Paladin - Pvp Retributon 4P bonus
        {
            ...
            ToPlayer()->AddSpellCooldown(31829, 0, 8000);
        ...
        case 110803:// Lightning Shield (Symbiosis)
        {
            ...
            ToPlayer()->AddSpellCooldown(110804, 0, 4000);
        ...
        case 81164:
        {
            ...
            ToPlayer()->AddSpellCooldown(81164, 0, 45000);
...
bool Unit::HandleOverrideClassScriptAuraProc(Unit* victim, uint32 /*damage*/, AuraEffectPtr triggeredByAura, SpellInfo const* /*procSpell*/)
{
...
uint32 Unit::SpellDamageBonusDone(Unit* victim, SpellInfo const* spellProto, uint32 pdamage, DamageEffectType damagetype, uint32 stack)
{
    ...
    if (GetTypeId() == TYPEID_PLAYER && HasAura(77220)
    ...
    if (GetTypeId() == TYPEID_PLAYER && damagetype == DOT && HasAura(77493))
    {
    ...
    if (GetTypeId() == TYPEID_PLAYER && HasAura(76547))
    {
        float Mastery = GetFloatValue(PLAYER_MASTERY) * 2.0f / 100.0f;
        float manapct = GetPowerPct(POWER_MANA);
        float bonus = 0;
    ...
    if (GetTypeId() == TYPEID_PLAYER && pdamage != 0 && spellProto->SchoolMask == SPELL_SCHOOL_MASK_FROST && HasAura(77514))
    {
    ...
    if (GetTypeId() == TYPEID_PLAYER && pdamage != 0 && spellProto->SchoolMask == SPELL_SCHOOL_MASK_SHADOW && HasAura(77515))
    {
    ...
    if (GetTypeId() == TYPEID_PLAYER && (spellProto->SchoolMask & SPELL_SCHOOL_MASK_FIRE || spellProto->SchoolMask & SPELL_SCHOOL_MASK_FROST || spellProto->SchoolMask & SPELL_SCHOOL_MASK_NATURE))
    {
        if (HasAura(77223))
        {
            float Mastery = GetFloatValue(PLAYER_MASTERY) * 2.0f;
    ...
    if (GetTypeId() == TYPEID_PLAYER && spellProto->SchoolMask == SPELL_SCHOOL_MASK_NATURE && HasAura(77492) && HasAura(48517)) // Solar Eclipse
    {
        float Mastery = GetFloatValue(PLAYER_MASTERY) * 1.87f;
        AddPct(DoneTotalMod, Mastery);
    }
    else if (GetTypeId() == TYPEID_PLAYER && spellProto->SchoolMask == SPELL_SCHOOL_MASK_ARCANE && HasAura(77492) && HasAura(48518)) // Lunar Eclipse
    {
        float Mastery = GetFloatValue(PLAYER_MASTERY) * 1.87f;
        AddPct(DoneTotalMod, Mastery);
    }
    ...
    if (GetTypeId() == TYPEID_PLAYER && (spellProto->Id == 116858 || spellProto->Id == 6353 || spellProto->Id == 104027))
    {
        float crit_chance;
    ...
    if (GetTypeId() == TYPEID_PLAYER && spellProto->Id == 11366 && damagetype == SPELL_DIRECT_DAMAGE && HasAura(48108))
        AddPct(DoneTotalMod, 25);

    // Fingers of Frost - 112965
    if (GetTypeId() == TYPEID_PLAYER && pdamage != 0 && ToPlayer()->GetSpecializationId(ToPlayer()->GetActiveSpec()) == SPEC_MAGE_FROST && getLevel() >= 24)
    {
    ...
    // Increases damage of Hammer of Wrath and Judgement too
     if (GetTypeId() == TYPEID_PLAYER && HasAura(53503) && ToPlayer()->IsTwoHandUsed() && (spellProto->Id == 20271 || spellProto->Id == 24275))
    ...
    else
    {
        ...
        switch (mtype)
        {
            case MOVE_WALK:
            {
                ...
                uint8 bitOrder[8] = {3, 0, 1, 5, 2, 7, 6, 4};
                data.WriteBitInOrder(guid, bitOrder);
...
int32 Unit::CalculateSpellDamage(Unit const* target, SpellInfo const* spellProto, uint8 effect_index, int32 const* basePoints, constAuraEffectPtr triggeredByAura) const
{
    return spellProto->Effects[effect_index].CalcValue(this, basePoints, target, triggeredByAura ? triggeredByAura->GetSpellInfo()->Id : 0);
}
...
void Unit::ProcDamageAndSpellFor(bool isVictim, Unit* target, uint32 procFlag, uint32 procExtra, WeaponAttackType attType, SpellInfo const* procSpell, uint32 damage, uint32 absorb, SpellInfo const* procAura)
{
    ...
    if (target && GetTypeId() == TYPEID_PLAYER && (procExtra & PROC_EX_CRITICAL_HIT) && HasAura(17007) && (attType == BASE_ATTACK || (procSpell && procSpell->GetSchoolMask() == SPELL_SCHOOL_MASK_NORMAL)))
    {
        if (!ToPlayer()->HasSpellCooldown(34299))
        {
            ...
            ToPlayer()->AddSpellCooldown(34299, 0, 6000); // 6s ICD
        }
    }
    ...
    if (target && target->GetTypeId() == TYPEID_PLAYER && target->HasAura(122464) && procSpell && procSpell->GetAllEffectsMechanicMask() & (1 << MECHANIC_STUN))
    {
        if (!target->ToPlayer()->HasSpellCooldown(122465))
        {
            ...
            target->ToPlayer()->AddSpellCooldown(122465, 0, 10000);
        }
    }
    ...
    for (ProcTriggeredList::const_iterator i = procTriggered.begin(); i != procTriggered.end(); ++i)
    {
        ...
        bool handled = false;
        if (HandleAuraProc(target, damage, i->aura, procSpell, procFlag, procExtra, &handled))
            takeCharges = true;
        ...
        if (!handled)
        {
            for (uint8 effIndex = 0; effIndex < MAX_SPELL_EFFECTS; ++effIndex)
            {
                ...
                if (i->aura->CallScriptEffectProcHandlers(triggeredByAura, aurApp, eventInfo))
                {
                    takeCharges = true;
                    continue;
                }
...
void Unit::Kill(Unit* victim, bool durabilityLoss, SpellInfo const* spellProto)
{
    ...
    else                                                // creature died
    {
        ...
        {
            Map* l_InstanceMap = creature->GetMap();
            Player* l_CreditedPlayer = GetCharmerOrOwnerPlayerOrPlayerItself();
            // TODO: do instance binding anyway if the charmer/owner is offline

            if (l_InstanceMap->IsRaidOrHeroicDungeon() && l_CreditedPlayer)
            {
