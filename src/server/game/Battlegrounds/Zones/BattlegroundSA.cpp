...
bool BattlegroundSA::ResetObjs()
{
    ...
    if (boatOne)
        boatOne->RemoveFromWorld();

    if (boatTwo)
        boatTwo->RemoveFromWorld();

    for (BattlegroundPlayerMap::const_iterator itr = GetPlayers().begin(); itr != GetPlayers().end(); ++itr)
        if (Player* player = ObjectAccessor::FindPlayer(itr->first))
            player->SetTransport(NULL);

    boatOne = CreateTransport(Attackers ? BG_SA_BOAT_ONE_H : BG_SA_BOAT_ONE_A, 0x220667CB, 2686.109f, -831.195f, 0.452f, 5.719f);
    boatTwo = CreateTransport(Attackers ? BG_SA_BOAT_TWO_H : BG_SA_BOAT_TWO_A, 0x220667CB, 2574.003f, 981.261f, 2.603f, 0.807f);

    boatOne->UpdateRotationFields(1.0f, 0.0002f);
    boatOne->SetUInt32Value(GAMEOBJECT_BYTES_1, 0xFF000B18);
    boatTwo->UpdateRotationFields(1.0f, 0.00001f);
    boatTwo->SetUInt32Value(GAMEOBJECT_BYTES_1, 0xFF000B18);
...
void BattlegroundSA::StartShips()
{
    ...
    boatOne->SetUInt32Value(GAMEOBJECT_LEVEL, 571066382);
    boatTwo->SetUInt32Value(GAMEOBJECT_LEVEL, 571066382);

    boatOne->SetUInt32Value(GAMEOBJECT_BYTES_1, 4278192921);
    boatTwo->SetUInt32Value(GAMEOBJECT_BYTES_1, 4278192921);
...
void BattlegroundSA::AddPlayer(Player* player)
{
    ...
    if (!ShipsStarted)
    {
        if (player->GetTeamId() == Attackers)
        {
            ...
            if (urand(0, 1))
            {
                player->TeleportTo(607, 2682.936f, -830.368f, 50.0f, 2.895f);
                if (boatOne)
                {
                    player->SetTransport(boatOne);
                    player->m_movementInfo.t_guid = boatOne->GetGUID();
                    player->m_movementInfo.t_seat = -1;
                    player->m_movementInfo.t_pos.m_positionX = -6.0f;
                    player->m_movementInfo.t_pos.m_positionY = -3.0f;
                    player->m_movementInfo.t_pos.m_positionZ = 0.0f;
                    player->m_movementInfo.t_pos.m_orientation = 8.6f;
                }
            }
            else
            {
                player->TeleportTo(607, 2577.003f, 980.261f, 50.0f, 0.807f);
                if (boatTwo)
                {
                    player->SetTransport(boatTwo);
                    player->m_movementInfo.t_guid = boatTwo->GetGUID();
                    player->m_movementInfo.t_seat = -1;
                    player->m_movementInfo.t_pos.m_positionX = -6.0f;
                    player->m_movementInfo.t_pos.m_positionY = -3.0f;
                    player->m_movementInfo.t_pos.m_positionZ = 0.0f;
    ...
    for (BattlegroundPlayerMap::const_iterator itr = GetPlayers().begin(); itr != GetPlayers().end(); ++itr)
    {
        if (Player* player = ObjectAccessor::FindPlayer(itr->first))
        {
            ...
            if (player->GetTeamId() == Attackers)
            {
                ...
                if (urand(0, 1))
                {
                    player->TeleportTo(607, 2682.936f, -830.368f, 50.0f, 2.895f, TELE_TO_NOT_LEAVE_TRANSPORT);
                ...
                {
                    player->TeleportTo(607, 2577.003f, 980.261f, 50.0f, 0.807f, TELE_TO_NOT_LEAVE_TRANSPORT);
...
void BattlegroundSA::ToggleTimer()
{
    ...
    if (TimerEnabled)
        UpdateWorldState(BG_SA_TIMER_END_TIMESTAMP, time(NULL) + (10 * MINUTE));
...
/*
    Some functions to handle transports (boat) in battleground
    We need to handle it manually, because trinity doesn't have
    a generic way to handle transport outside continent maps
 */
...
Transport* BattlegroundSA::CreateTransport(uint32 goEntry, uint32 period, float x, float y, float z, float o)
{
    ...
    if (!t->Create(sObjectMgr->GenerateLowGuid(HIGHGUID_MO_TRANSPORT), goEntry, 607, x, y, z, o, 255, 0))
    {
        delete t;
        return NULL;
    }
