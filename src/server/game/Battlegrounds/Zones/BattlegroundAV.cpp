...
WorldSafeLocsEntry const* BattlegroundAV::GetClosestGraveYard(Player* player)
{
    ...
    minDist = (pGraveyard->m_PositionX - x)*(pGraveyard->m_PositionX - x)+(pGraveyard->m_PositionY - y)*(pGraveyard->m_PositionY - y);
    ...
    for (uint8 i = BG_AV_NODES_FIRSTAID_STATION; i <= BG_AV_NODES_FROSTWOLF_HUT; ++i)
        if (m_Nodes[i].Owner == player->GetTeam() && m_Nodes[i].State == POINT_CONTROLED)
        {
            ...
            if (entry)
            {
                dist = (entry->m_PositionX - x)*(entry->m_PositionX - x)+(entry->m_PositionY - y)*(entry->m_PositionY - y);
                if (dist < minDist)
                {
                    minDist = dist;
                    pGraveyard = entry;
