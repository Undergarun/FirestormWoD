...
void MotionMaster::MoveTargetedHome()
{
    ...
    else if (_owner->GetTypeId()==TYPEID_UNIT && ((Creature*)_owner)->GetCharmerOrOwnerGUID())
    {
        ...
        if (target)
        {
            sLog->outDebug(LOG_FILTER_GENERAL, "Following %s (GUID: %u)", target->IsPlayer() ? "player" : "creature", target->IsPlayer() ? target->GetGUIDLow() : ((Creature*)target)->GetDBTableGUIDLow());
...
void MotionMaster::MoveChase(Unit* target, float dist, float angle)
{
    ...
    if (_owner->IsPlayer())
    {
        sLog->outDebug(LOG_FILTER_GENERAL, "Player (GUID: %u) chase to %s (GUID: %u)",
            _owner->GetGUIDLow(),
            target->IsPlayer() ? "player" : "creature",
            target->IsPlayer() ? target->GetGUIDLow() : target->ToCreature()->GetDBTableGUIDLow());
...
void MotionMaster::MovePoint(uint32 id, float x, float y, float z, float p_Orientation)
{
    if (_owner->IsPlayer())
    {
        sLog->outDebug(LOG_FILTER_GENERAL, "Player (GUID: %u) targeted point (Id: %u X: %f Y: %f Z: %f)", _owner->GetGUIDLow(), id, x, y, z);
        Mutate(new PointMovementGenerator<Player>(id, x, y, z, 0.0f, p_Orientation), MOTION_SLOT_ACTIVE);
    }
    else
    {
        ...
        Mutate(new PointMovementGenerator<Creature>(id, x, y, z, 0.0f, p_Orientation), MOTION_SLOT_ACTIVE);
    }
}
