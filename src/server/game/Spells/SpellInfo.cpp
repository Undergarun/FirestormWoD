...
int32 SpellEffectInfo::CalcValue(Unit const* caster, int32 const* bp, Unit const* target, uint32 auraId) const
{
    ...
    if (scaling && scaling->Multiplier != 0.0f)
    {
        if (caster && !_spellInfo->IsCustomCalculated())
        {
            ...
            // Hack Fix Rejuvenation, doesn't use the target level for basepoints
            if (target && _spellInfo->IsPositiveEffect(_effIndex) && (Effect == SPELL_EFFECT_APPLY_AURA || Effect == SPELL_EFFECT_APPLY_AURA_2) && _spellInfo->Id != 774)
...
SpellCastResult SpellInfo::CheckExplicitTarget(Unit const* caster, WorldObject const* target, Item const* itemTarget) const
{
    ...
    if (!target)
    {
        if (neededTargets & (TARGET_FLAG_UNIT_MASK | TARGET_FLAG_GAMEOBJECT_MASK | TARGET_FLAG_CORPSE_MASK) && Id != 115073 && Id != 6544 && Id != 115460)
