...
public:
    ...
    void LogSpell(uint32 p_SpellID, char* p_String, ...);
    void LogSpell(uint32 p_SpellID, uint32 p_SpellID2, char* p_String, ...);
...
#define LOG_SPELL(s, fmt, ...) sSpellLogMgr->LogSpell(s, fmt, ##__VA_ARGS__)
#define LOG_SPELL_DOUBLE(s1, s2, fmt, ...) sSpellLogMgr->LogSpell(s1, s2, fmt, ##__VA_ARGS__)
