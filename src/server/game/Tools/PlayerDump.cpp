...
DumpReturn PlayerDumpReader::LoadDump(const std::string& p_File, uint32 p_Account, std::string p_Name, uint32 p_Guid, bool p_OnlyBoundedItems, uint32 p_AtLogin)
{
    ...
    while (!feof(fin))
    {
        ...
        switch (l_TableType)
        {
            ...
            default:
                ...
                {
                    sLog->outSlack("#jarvis", "danger", "Transfer to realm [%u] on account [%u] failed. Reason: Unknow table", g_RealmID, p_Account);
                    ROLLBACK(DUMP_FILE_BROKEN);
                }
                break;
        }
    ...
    {
        sLog->outSlack("#jarvis", "danger", "Transfer to realm [%u] on account [%u] failed. Reason: Character transaction fail", g_RealmID, p_Account);
