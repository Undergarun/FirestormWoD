...
void WorldDatabaseConnection::DoPrepareStatements()
{
    ...
    PREPARE_STATEMENT(WORLD_INS_CREATURE, "INSERT INTO creature (guid, id , map, zoneId, areaId, spawnMask, phaseMask, modelid, equipment_id, position_x, position_y, position_z, orientation, spawntimesecs, spawndist, currentwaypoint, curhealth, curmana, MovementType, npcflag, npcflag2, unit_flags, unit_flags2, dynamicflags, isActive) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", CONNECTION_ASYNC);
    ...
    PREPARE_STATEMENT(WORLD_SEL_AREATRIGGER_VISUAL, "SELECT entry, visual_radius FROM areatrigger_visual", CONNECTION_SYNCH);
}
